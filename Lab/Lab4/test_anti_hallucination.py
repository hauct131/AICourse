...
def main() -> int:
    ...
    answer = str(in_scope.get("answer", "")).lower()
    if (
        not answer
        or in_scope.get("answer") == UNKNOWN
        or not in_scope.get("sources")
        or any(marker in answer for marker in REFUSAL_MARKERS)
    ):
        print("FAIL: Câu trong phạm vi chưa được trả lời đúng.", file=sys.stderr)
    ...
    print("PASS: in-scope trả lời từ tài liệu; out-of-scope đều abstain.")
    return 0
